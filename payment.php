...
<div style="max-width:900px;margin:0 auto">
  <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;flex-wrap:wrap">
    <a href="<?= $payments_base ?><?= $type==='new'?'guest':($type==='enrolled'?'guestold_student':'guestold') ?>" class="btn" style="background:#fff;border:1px solid var(--line); padding:7px 12px; font-size:12px">Back</a>
...
document.getElementById('payForm')?.addEventListener('submit', function(e){
  // build description first, then let Google-Forms handler validate
  buildDesc();
  const firstInvalid = (function(){
    const checks = [
      ...
      {id:'contact_no', fieldId:'field-contact'},
...
(function(){
  ...
  function syncParticularsRequired(){
    ...
    // if custom typed, dropdown no longer required; otherwise required
    if(othHas){
...
// Sequential field guidance — indicated (active) field is visually dominant
(function(){
  const order = [
    ...
    { fieldId:'field-payee', inputs:['payee_name'], label:'Student / Payer Name' },
  ...
  function clearSeq(){
    ...
    document.querySelectorAll('.seq-dim').forEach(e=>e.classList.remove('seq-dim'));
    document.querySelectorAll('.seq-done').forEach(e=>e.classList.remove('seq-done'));
  ...
  function updateSeq(){
    ...
    for(let i=0;i<order.length;i++){
      if(!isFilled(order[i])){ firstEmpty=order[i]; firstEmptyIdx=i; break; }
    }
    const newId = firstEmpty ? firstEmpty.fieldId : null;
    // refresh dim/done states even if active id unchanged (to reflect fill vs done)
    if(newId !== currentSeqId){
      clearSeq();
      currentSeqId = newId;
      if(!firstEmpty) {
        // all done — mark everything as done (no dim)
        order.forEach(e=>{
          const c=document.getElementById(e.fieldId);
          if(c && isFilled(e)) c.classList.add('seq-done');
        });
      ...
      // dim / done siblings so active field visually dominates
      order.forEach((e, idx)=>{
        if(e.fieldId===firstEmpty.fieldId) return;
        const c=document.getElementById(e.fieldId);
        if(!c) return;
        if(idx < firstEmptyIdx && isFilled(e)){
          c.classList.add('seq-done');
        } else {
          c.classList.add('seq-dim');
        }
      });
      ...
      if(label && !label.querySelector('.seq-badge')){
        ...
        badge.textContent = firstEmptyIdx===0 ? ' Start here \u2192' : ' Next \u2192';
      ...
      if(!container.querySelector('.seq-hint')){
        ...
        hint.textContent = firstEmptyIdx===0 ? 'Fill this field first to continue' : 'Required — fill this next (you may skip, but this is recommended order)';
        container.appendChild(hint);
      }
      if(window._updatePayGate) window._updatePayGate();
    } else {
      // same active field — just refresh done/dim for siblings (e.g. after typing)
      order.forEach((e, idx)=>{
        const c=document.getElementById(e.fieldId);
        if(!c || c.classList.contains('seq-next')) return;
        c.classList.remove('seq-dim','seq-done');
        if(idx < firstEmptyIdx && isFilled(e)) c.classList.add('seq-done');
        else if(firstEmpty) c.classList.add('seq-dim');
      });
    }
  }
  ...
  // expose for other gates to refresh prominence
  window._updateSeq = updateSeq;
})();
</script>

<script>
// Google Forms-style: hint click + native invalid -> scroll to field
(function(){
  const hint=document.getElementById('payBtnHint');
...
/* keep grey-out of non-indicated fields */
.seq-dim{opacity:.48; filter:saturate(.75) brightness(.98)}
.seq-dim input,.seq-dim select{background:#f8fafc !important;border-color:#e5e9f2 !important;box-shadow:none !important}
.seq-dim label{color:#64748b !important}
.seq-done{opacity:.78; filter:none}
.seq-done input,.seq-done select{border-color:#a7f3d0 !important;background:#f0fdf4 !important}
.seq-done label{color:#065f46 !important}
</style>
