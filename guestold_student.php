...
if(isset($_POST["btnsubmit"])){
    ...
    $name=findStudentByNumber($con,$studno,$campid);
    if($name && $studno!==''){
        header("Location: " . $payments_base . "payment_oldstud.php?payee=".urlencode($name)."&transid=".urlencode($transid)."&studentno=".urlencode($studno)."&type=enrolled&campus=".urlencode($campid));
        exit;
    } else {
