...
<link rel="stylesheet" href="<?= $payments_base ?>assets/style.css?v=21">
</head>
<body>
<header class="pay-header">
  <div class="pay-header-inner">
    ...
    <nav class="pay-nav" id="payNav">
      <a href="<?= $payments_base ?>guest" class="pay-nav-link <?= in_array($current,['guest','new-enrollee'])?'active':'' ?>">New Enrollees</a>
      <a href="<?= $payments_base ?>guestold_student" class="pay-nav-link <?= in_array($current,['guestold_student','enrolled'])?'active':'' ?>">Enrolled</a>
      <a href="<?= $payments_base ?>guestold" class="pay-nav-link <?= in_array($current,['guestold','other'])?'active':'' ?>">Other</a>
    ...
    <div style="display:flex;align-items:center;gap:8px;flex-shrink:0">
      <a href="<?= $payments_base ?>instructions" class="pay-howtopay-persist <?= $current==='instructions'?'active':'' ?>" aria-label="How to Pay"><i class="fa-solid fa-circle-question"></i><span>How to Pay</span></a>
